...
function setMobiusURL() {
  ...
  var in_port;
  while (true) {
    mobiusURL = input.question(
      "변경할 Mobius의 주소를 입력해주세요(예 : www.mobius.com or 192.168.0.1) : "
    );
    in_port = input.question("Mobius의 포트 번호를 입력해주세요 : ");

    if (
      (ValidIpAddressRegex.test(mobiusURL) ||
        ValidHostnameRegex.test(mobiusURL)) &&
      !isNaN(in_port)
    )
      break;
...
function modifyMobiusSetting(source, service, host, mqtt, port) {
  ...
  if (!config.mobius)
    if (!config) return reject(new Error("Invalid CONFIG source"));
    else return source;
...
function saveConfig(source, service) {
  ...
  fs.writeFileSync(
    `${CONFIGS[service].packageLocation}`,
    configStr,
    "utf8",
    function (err) {
      if (err) {
...
function getPort(service, origin) {
  while (true) {
    var port = input.questionInt(
      `서비스 ${service}에 할당할 포트 번호를 입력해주세요(기본값 : ${origin}) : `
    );
    if (!port && port <= 0) {
...
function setServicePort() {
  return new Promise(function (resolve, reject) {
    ...
    var temp = [];
    while (true) {
      var choise = input.keyInSelect(
        serviceNames,
        "포트번호를 변경할 서비스를 선택해주세요."
      );
  ...
  return new Promise(function (resolve, reject) {
    try {
      ...
      if (useSub) {
        ...
        var names = subDomains.map((el) => {
          return `${el.service} => ${el.subdomain}`;
        });
        names.push("next");
        while (true) {
          var choise = input.keyInSelect(
            names,
            "서브도메인을 변경할 서비스를 선택해주세요 : "
          );
          ...
          if (choise == -1) {
            ...
            if (input.keyInYN("서비스 도메인을 위와 같이 사용하시겠습니까?"))
              break;
          } else {
            names[
              choise
            ] = `${subDomains[choise].service} => ${subDomains[choise].subdomain}`;
          }
        }
        domainList = {};
...
gulp.task("domainList", function () {
  return new Promise(function (resolve, reject) {
    try {
      ...
      if (!domain) {
        console.error(
          "플랫폼 설정이 완료되지 않았습니다. 'gulp init'으로 설정을 마쳐주세요"
        );
        resolve();
      } else {
        var mainDomain = json[0].config.default.cookie
          ? json[0].config.default.cookie.domain
          : null;
