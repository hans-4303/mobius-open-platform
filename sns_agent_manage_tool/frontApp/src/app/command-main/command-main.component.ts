...
export class CommandMainComponent implements OnInit {
  ...
  loadComponent(bot, i) {
    ...
    let reqFactory =
      this.componentFactoryResolver.resolveComponentFactory(BotViewComponent);
    ...
    this.dialog
      ...
      .subscribe(
        (newBot) => {
          ...
          this.botService
            .addNewBot(this.userEmail, newBot)
            .pipe(
              map((rs) => {
                return rs.data;
              }),
            ...
            this.botService.deleteBot(bot.botID).subscribe(
              ...
              (err) => {
                this.notifierService.notify(
                  "error",
                  err.error.message || "삭제 중 장애가 발생했습니다."
                );
              }
            );
          }
...
export class DialogBotAdd {
  ...
  onClick() {
    if (
      ...
      this.form.controls["token"].valid
    ) {
      if (this.form.valid)
...
export class BotRegisterGuide {
  constructor(public dialogRef: MatDialogRef<BotRegisterGuide>) {}
}
