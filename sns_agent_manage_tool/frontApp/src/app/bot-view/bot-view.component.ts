...
export class BotViewComponent implements OnInit {
  ...
  isAllSelected(table) {
    const numSelected = table
      ? this.requestSelection.selected.length
      : this.userSelection.selected.length;
  ...
  onClickToValidUser() {
    if (!this.requestSelection.hasValue()) {
      return;
    }
  ...
  onClickToRequest() {
    if (!this.userSelection.hasValue()) {
      return;
    }
    let target = new Array();
  ...
  onClickRemove() {
    if (!(this.requestSelection.hasValue() || this.userSelection.hasValue())) {
      return;
    }
    this.dialog
      .open(DialogBotListDelete, {
        width: "250px",
      })
      .afterClosed()
      .subscribe((rs) => {
        ...
        //삭제 날리기
        this.botService
          .removeRequester(this.botID, validTarget, requestTarget)
          .subscribe(
            (rs) => {
...
export class DialogBotListDelete {
  constructor(public dialogRef: MatDialogRef<DialogBotListDelete>) {}
}
