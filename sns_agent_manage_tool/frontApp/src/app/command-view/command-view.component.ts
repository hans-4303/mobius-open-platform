...
export class CommandViewComponent implements OnInit {
  ...
  constructor(
    ...
    private cmdServcie: CommandService,
  ...
  getCommandByBotID() {
    ...
    this.cmdServcie.getCommands(this.botID).subscribe(
      (data) => {
        if (data.commands) {
  ...
  toggleCommandActivity(i) {
    this.selection.clear();
    this.cmdServcie
      .toggleCommandActivity(this.botID, this.commandList[i].cmdID)
      .subscribe(
        (res: any) => {
          let data = res.data;
    ...
    this.dialog
      ...
      .subscribe(
        (res) => {
          ...
          this.ngxService.show();
          this.cmdServcie.addCommand(this.botID, res).subscribe(
            (res: any) => {
              let data = res.data;
          ...
          this.cmdServcie.deleteCommands(this.botID, cmdIDs).subscribe(
            ...
            (err) => {
              this.notifier.notify(
                "error",
                err.message || "복수의 명령어 삭제에 실패했습니다."
              );
            },
            () => {
              this.ngxService.hide();
    ...
    this.dialog
      ...
      .subscribe(
        (res) => {
          ...
          this.ngxService.show();
          this.cmdServcie
            .deleteCommandActivity(this.botID, row.cmdID)
            .subscribe(
              (rs) => {
...
export class DialogAdd {
  ...
  constructor(
    ...
    this.form = this.fb.group({
      command: [
        "",
        [Validators.required, Validators.pattern("^[/?a-zA-Z0-9_]{0,}$")],
      ],
  ...
  onClick = () => {
    ...
    if (
      this.form.controls["command"].valid &&
      this.form.controls["target"].valid &&
      this.form.controls["method"].valid
    ) {
