...
class MobiusTelegramBot {
      ...
      botModel
        ...
        .catch((err) => {
          this.bot.sendMessage(
            msg.chat.id,
            `명령어 실행에 실패했습니다. 지속시 관리자에게 문의 바랍니다.\n ${err.message}`
          );
        });
    });
  }
  ...
  setRequest() {
    this.bot.onText(/\/request/, (msg) => {
      ...
      requestModel
        .findOne({ _owner: this.botID, chatID: chatID })
        .exec()
        .then((rs) => {
          if (!rs) {
            validModel
              ...
              .catch((err) => {
                ...
                this.bot.sendMessage(
                  chatID,
                  "사용 요청 도중 장애가 발생했습니다. 지속시 관리자에게 문의 바랍니다."
                );
              });
          } else {
            this.bot.sendMessage(
  ...
  setStopSub() {
    this.bot.onText(/^\/stop_([a-zA-Z0-9_]\w+)$/, (msg, match) => {
      var command = match[1];
      commandModel
        .findOne({ _owner: this.botID, command: command })
        .lean()
        .exec()
        .then((cmd) => {
          if (cmd) {
    ...
    if (cmdInfo.method.toLowerCase() === "get") {
      ...
      operate = (msg, match) => {
        if (this.isValid(msg.chat.id)) {
          if (match.length < 2) {
            ...
            cmdFn.setPost(match[1], system_origin, (err, res) => {
              if (err) {
                ...
                this.bot.sendMessage(
                  msg.chat.id,
                  "명령어 등록에 실패했습니다. 지속시 관리자에게 문의 바랍니다."
                );
              } else if (res) {
                this.bot.sendMessage(
                  msg.chat.id,
                  `${cmdInfo.target} <= ${res["m2m:cin"].con} 전달`
                );
              }
            });
          }
          ...
          this.bot.sendMessage(
            msg.chat.id,
            "/request 명령어로 봇 관리자에게 승인 뒤 이용이 가능합니다."
          );
        }
      };
      regexCmd = new RegExp(`\/${cmdInfo.command} (((.|\\n|\\r)*))`);
      ...
      cmdFn
        .setSub(`${global.CONFIG.mobius_sub_prefix}_SUB`, system_origin)
        .then((mqttClient) => {
          if (!mqttClient) {
      ...
      operate = (msg) => {
        try {
          ...
          this.bot.sendMessage(
            msg.chat.id,
            `${cmdInfo.target} 데이터를 구독합니다.`
          );
        } catch (err) {
          this.bot.sendMessage(
            msg.chat.id,
            `명령어 실행이 준비되지 않았습니다. 잠시후에 시도해주세요.`
          );
        }
      };
      regexCmd = new RegExp(`^\/${cmdInfo.command}$`);
