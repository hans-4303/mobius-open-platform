...
module.exports = function () {
  return new Promise((resolve, reject) => {
    try {
      var aeURL =
        `${CONFIG.mobius.host}:${CONFIG.mobius.port}/${CONFIG.mobius.csebase}` +
        "/" +
        CONFIG.system_name;
      ...
      onem2mClient.Http.createResource(parentResourcePath, SERVICE_AE, origin)
        ...
        .catch(function (err) {
          ...
          var resourceUrl =
            parentResourcePath + "/" + SERVICE_AE["m2m:ae"]["rn"];
          var cseOrigin = "/" + target.cseName; //  AE가 생성되었지만 아직 aei를 알지 못하는 상태기 때문에 origin을 CSE로 하여 조회
          onem2mClient.Http.getResource(resourceUrl, cseOrigin).then(
            function (res) {
              resolve(res["m2m:ae"]["aei"]);
            },
            function (err) {
