...
exports.createUser = (req, res) => {
  ...
  encrypt
    .encryptSHA512(req.body.password)
    .then((encrypted) => {
...
exports.modifyUser = (req, res) => {
  if (!hasOwnProperties(req.params, "email", "password", "name", "token")) {
    sendError(
      res,
      null,
      "정확하지 않은 정보들이 전달되었습니다. 다시 입력해주세요"
    );
    return;
  }
  ...
  UserModel.deleteUsers(target)
    .then((result) => {
      botModule
        ...
        .then((botIDs) => {
          validModel
            .deleteValidsByOwner(botIDs)
            .then(() => {
              requestModel
                .deleteRequestsByOwner(botIDs)
                .then(() => {
                  commandModel
                    .deleteCommandsByOnwer(botIDs)
                    .then(() => {
                      botIDs.forEach((bot) => {
...
const sendError = (res, err, content) => {
  ...
  res.status(401).json({ message: content });
};
