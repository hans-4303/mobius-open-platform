...
exports.addCmd = (req, res) => {
  ...
  CommandModel.checkOverlap(botID, body.command)
    .then((checkOverlap) => {
      ...
      onem2mService
        .checkExistTarget(body.target, botID)
        .then((rs) => {
        ...
        .catch((err) => {
          sendFailMessage(
            res,
            err,
            `${body.target}이 Mobius에 존재하지 않습니다.`
          );
        });
    })
    .catch((err) => {
...
exports.deleteCommands = (req, res) => {
  ...
  CommandModel.deleteCommands(botID, cmdIDs).then((result) => {
    if (result) {
      result.forEach((el) => {
  ...
  try {
    ...
    CommandModel.toggleCommandActivity(botID, cmdID)
      .then((result) => {
        result = result._doc;
        if (result) {
          // command 모델이 없으면 불일치한 정보
          if (result.activity) {
            // 명령어 상태를 확인, botID와 command정보를 통째로 넘긴다
