    ...
    userModel
      ...
      .catch((err) => {
        sendFailMessage(
          res,
          err,
          "봇 조회 도중 장애가 발생했습니다. 지속시 관리자에게 문의 바랍니다."
        );
      });
  } catch (err) {
    sendFailMessage(
      ...
      validModel
        ...
        .catch((err) => {
          sendFailMessage(
            res,
            err,
            "사용자 권한 변환 도중 문제가 발생했습니다."
          );
        });
    }
    if (request) {
      requestModel.removeRequest(request).catch((err) => {
