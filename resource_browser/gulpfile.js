...
gulp.task("copy-source", function () {
  ...
  return gulp.src(srcFiles, { base: "." }).pipe(gulp.dest(nwfiles));
});
gulp.task("copy-package", function () {
  var srcFiles = [];
  srcFiles.push("public/package.json");

  return gulp.src(srcFiles).pipe(gulp.dest(nwfiles));
});
...
gulp.task("copy-modules", function () {
  ...
  return gulp
    .src(moduleFiles, { base: "node_modules" })
...
options.winIco
  ...
  nw.on("log", function (msg) {
    if (!msg.startsWith("Zipping")) gutil.log("nw-builder", msg);
  });
...
gulp.task("default", function () {
  ...
  runSequence(
    "clean-nwfiles",
    ["copy-source", "copy-package", "copy-modules"],
    "nw"
  );
});
