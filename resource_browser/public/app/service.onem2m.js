...
(function () {
  ...
  function Onem2mService($http, store) {
    ...
    function requestOverHttp(option, cbSuccess, cbError) {
      $http(option).then(cbSuccess, cbError);
    }
    ...
    function startHttpRequestQueue() {
      setInterval(function () {
        ...
        else runningJob = HTTP_REQUEST_JOB_QUEUE.shift();
    ...
    function subscribeTo(targetResourceUrl, subName, origin, mqttUrl) {
      return new Promise(function (resolve, reject) {
        ...
        if (origin.startsWith("/")) notificationUrl += origin + "?ct=json";
        else notificationUrl += "/" + origin + "?ct=json";
  ...
  function getCB2(url, cb) {
    ...
    $http({
      url:
        window.API_BASE_URL + "/onem2m/" + encodeURIComponent(url + "/" + cb),
      method: "GET",
      headers: {
