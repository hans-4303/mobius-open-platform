(function () {
  ...
  CreateResourceModalController.$inject = [
    "$scope",
    "store",
    "parentNode",
    "close",
...
*/
    ...
    var contentModels =
      window.OneM2M.Resource.getContentModel(parentResourceType);
    for (var i = 0; i < contentModels.length; i++) {
      $scope.contentModel.push({
        typeCode: contentModels[i],
        iconUrl: RESOURCE_IMAGES[contentModels[i]],
      });
    }
    ...
    function closeModal(result) {
      if (result == false) return close(null, 500); //  close, but give 500ms for bootstrap to animate

      var aryLbl = [];
      if ($scope.resource.lbl) aryLbl = $scope.resource.lbl.split(" ");
      ...
      switch ($scope.model.selectedResourceType) {
        ...
        case "23":
          var net = [];
          if ($scope.resource.enc1) net.push("1");
          if ($scope.resource.enc2) net.push("2");
          if ($scope.resource.enc3) net.push("3");
          if ($scope.resource.enc4) net.push("4");
