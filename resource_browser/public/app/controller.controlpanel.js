(function () {
  ...
  function ControlPanelController(
    ...
    function initScope() {
      ...
      else $scope.resourcePath = store.get("resourcePath");

      if (!$scope.resourceHost)
        $scope.resourceHost = "http://203.253.128.161:7579";
      if (!$scope.resourcePath) $scope.resourcePath = "/Mobius/justin";
      ...
      if ($scope.resourceUrlList == null) $scope.resourceUrlList = [];

      $scope.resourceHostList = store.get("resourceHostList");
      if ($scope.resourceHostList == null) $scope.resourceHostList = [];
      ...
      if ($scope.numberOfInstances == null) $scope.numberOfInstances = 3;
      ...
      eventService.on(
        ...
        monitoringStateChangedHandler
      );
    }

    function toggleControlPanel(forceOpen) {
      setTimeout(function () {
        if (forceOpen) $("#control-panel").removeClass("collapsed");
        else $("#control-panel").toggleClass("collapsed");
      }, 100);
    }
    ...
    function validateInputValues(scope) {
      if (scope.resourcePath == null) scope.resourcePath = scope.searchText;
      if (scope.resourceHost == null) scope.resourceHost = scope.searchHostText;
    ...
    function monitoringStateChangedHandler(event, state) {
      if (state == "starting") eventService.showWaitforDialog();
      else eventService.hideWaitforDialog();

      console.log(
        "MONITORING STATE CHANGED from " + $scope.state + " to " + state
      );
      switch (state) {
        case "stopped":
          $scope.startstopChecked = false;
    ...
    function updateAndSaveArrayValue(key, array, updateValue, fixedCount) {
      ...
      if (array.length > 10) array.splice(10, 100);
    ...
    function resourceHostFilter(keyword) {
      ...
      return $scope.resourceHostList.filter(function (item) {
        ...
        if (!result) {
          console.log(
            angular.$$lowercase(item),
            lowcaseFilter,
            angular.$$lowercase(item).indexOf(lowcaseFilter)
          );
        }
