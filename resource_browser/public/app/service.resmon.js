...
(function () {
  ...
  function ResmonService($http, $rootScope, eventService, onem2mService) {
    ...
    ResourceNode.prototype.appendChild = function (nodeName) {
      if (this.mapChildren[nodeName]) return this.mapChildren[nodeName];
    ...
    ResourceNode.prototype.compare = function (other) {
      function getLtOrCt(data) {
        ...
        if (resourceType == "m2m:cin") {
          ...
          else return data[resourceType].ct;
        } else {
      ...
      if (this.data && other.data) {
        ...
        if (ltLeft && ltRight) {
          ...
          if (result == 0) return localeCompare(this.name, other.name);
          else return result;
        } else {
    ...
    ResourceNode.prototype.removeChild = function (nodeName, onlyChildList) {
      ...
      if (node) {
        ...
        if (!onlyChildList) {
          ...
          if (resourceId && nodeMap[resourceId]) delete nodeMap[resourceId];
        }
    ...
    ResourceNode.prototype.setData = function (nodeData, updated) {
      ...
      var parentId = null;
      if (this.parent) parentId = this.parent.getResourceId();
      ...
      if (
        resourceType == "m2m:cin" ||
        resourceType == "cin" ||
        resourceType == "m2m:sub" ||
        resourceType == "sub"
      )
      ...
      if (this.parent) this.parent.limitInstanceCount();
      this.limitInstanceCount();
    };
    ...
    ResourceNode.prototype.collapse = function () {
      if (this.collapsed || !this.discovered) return;
    ...
    ResourceNode.prototype.hasChildren = function () {
      if (this.aryChildren && this.aryChildren.length > 0) return true;
      ...
      if (instanceTypeCode != "") {
        ...
        for (var i = 0; i < this.aryChildren.length; i++) {
          if (this.aryChildren[i].getTypeCode() == instanceTypeCode) {
            nInstanceCount++;
            if (nInstanceCount > 5) {
              //ResourceNode.INSTANCE_COUNT_LIMIT ) {
    ...
    function findNode(pathOrId) {
      if (pathOrId.indexOf("/") == -1) {
        ...
        if (nodeMap[pathOrId]) return nodeMap[pathOrId];
        else return null;
      } else {
