...
(function () {
  ...
  if (!window.OneM2M) window.OneM2M = {};
  ...
  function getAccessPointInfo(url) {
    ...
    var cseName = "";
    if (aryPath.length > 0) cseName = aryPath[0];
    ...
    if (query) {
      ...
      for (var i = 0; aryQueryItems && i < aryQueryItems.length; i++) {
        ...
        if (keyValue.length > 0) key = keyValue[0].trim();

        if (key.length == 0) continue;

        if (keyValue.length > 1 && keyValue[1].length > 0) value = keyValue[1];
  ...
  var CONTENT_MODEL = {
    1: [], //  acp - sub
    2: ["3", "23", "24"], //  ae - sub, container, group, acp, pollingChannel, schedule, semanticDesc, timeSeries
    3: ["4", "3", "23", "24"], // cnt - contentInstance, sub, contain, latest, oldest, semanticDesc,
    4: [], // cin  - semanticDesc
    5: [], //  cb  - ae, container, group, acp, sub, timeseries
    9: ["23", "24"], //  grp  0 sub, semanticDesc
    10: [], //  lcp
    16: ["23"], //  csr   -- container, group, acp, subscription, timeseries
    23: [], //  sub
    24: ["23"], //  sd  - sub
    25: ["23", "24"], //  ts   - timeSeriesInstance, sub, semanticDesc
    26: [], //  tsi
    27: [], //  mms
    99: [], //  rsp
  };
