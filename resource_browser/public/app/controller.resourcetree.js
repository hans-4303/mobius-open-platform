(function () {
   ...
   */
  function ResourceTreeController(
    $scope,
    $rootScope,
    $mdDialog,
    store,
    ...
    function canZoomIn() {
      if ($scope.zoom == undefined) return true;

      if ($scope.zoom.scale() >= MAX_ZOOM_SCALE) return true;
      else return false;
    }
    ...
    function canZoomReset() {
      ...
      if ($scope.zoom.scale() == 1) return true;
      else return false;
    }
    ...
    function canZoomOut() {
      ...
      if ($scope.zoom.scale() <= MIN_ZOOM_SCALE) return true;
      else return false;
    }
          ...
          modal.close.then(function (deleteInfo) {
            if (deleteInfo) {
              ...
              if (deleteInfo.resourceId) {
                ...
                var resourceUrl =
                  resourceMonitor.monitoringTarget.baseUrl + resourceId;
                var acpOrigin = resourceMonitor.acpOrigin;
                if (deleteOriginator) acpOrigin = deleteOriginator;

                onem2mService
                  .deleteResource(resourceUrl, acpOrigin)
                  .then(function (res) {
                    var newResourceType = _.keys(res)[0];
                    var parentNode = resmonService.findNode(
                      res[newResourceType].pi
                    );
                    if (parentNode) {
    ...
    function initScope() {
      ...
      eventService.on(
        $rootScope,
        "monitoring.get.resource",
        onGetResourceRequest
      );
      eventService.on(
        $rootScope,
        "monitoring.discover.resource",
        onDiscoverResourceRequest
      );
      eventService.on(
        $rootScope,
        "monitoring.delete.resource",
        onDeleteResourceRequest
      );
      eventService.on(
        $rootScope,
        "monitoring.update.resource",
        onUpdateResourceRequest
      );
      eventService.on(
        $rootScope,
        "monitoring.update.animation",
        onUpdateAnimationEffect
      );
      eventService.on(
        $rootScope,
        "monitoring.created.resource",
    ...
    function onDiscoverResourceRequest(event, args) {
      var resourceUrl =
        resourceMonitor.monitoringTarget.baseUrl + args.resourcePath;
      var node = args.node;
      var acpOrigin = resourceMonitor.acpOrigin;
    ...
    function onGetResourceRequest(event, args) {
      ...
      var resourceUrl = resourceMonitor.monitoringTarget.baseUrl + path;
      onem2mService
        .getResource(resourceUrl, acpOrigin)
        .then(function (res) {
    ...
    function onMonitoringStartRequest(event, args) {
      ...
      resourceMonitor.monitoringTarget = OneM2M.Util.getAccessPointInfo(
        args.resourceUrl
      );
      resourceMonitor.targetResourceDepth =
        resourceMonitor.monitoringTarget.aryPaths.length;
      ...
      createOrGetResourceMonitorAE(
        resourceMonitor.monitoringTarget,
        resourceMonitor.resourceMonitorAE
      )
        .then(function (aeObj) {
          ...
          if (!aeID || aeID.trim().length == 0) {
            ...
            console.log(
              "failed to create or get resource monitoring AE. try to create AE but aeid is null"
            );

            eventService.emit($rootScope, "monitoring.statechanged", "stopped");
            eventService.showError(
              "대상 서버에 접속할 수 없습니다. (Failed to create browser AE resource)"
            );
            ...
            var resourceUrl =
              resourceMonitor.monitoringTarget.baseUrl +
              ...
              .then(
                function (res) {
                  ...
                  socket.on("connected", function (data) {
                    ...
                    socket.emit("start", {
                      to: mqttAddress,
                      path: resourceMonitor.monitoringTarget.path,
                      aeId: acpOrigin,
                    });
                  });
                  ...
                  socket.on("mqtt:notification", function (data) {
                    ...
                    try {
                      var sgn =
                        data.primitiveContent["sgn"] ||
                        data.primitiveContent["m2m:sgn"];
                      ...
                      switch (notiEventType) {
                        ...
                        case "1": // update of resource
                          var eventNodeId = arySur
                            .splice(0, arySur.length - 1)
                            .join("/");
                        ...
                        case "2": // delete of resource
                          ...
                          if (parentNode) {
                            parentNode.removeChild(
                              representation[resourceType].rn
                            );
                  ...
                  console.log(
                    "check wether given resource path is valid or not: valid"
                  );

                  resmonService.initResourceTree(
                    resourceMonitor.monitoringTarget.path
                  );
    ...
    function getBaseAEResource(resourcePath, origin) {
      return new Promise(function (resolve, reject) {
        onem2mService.getResource(resourcePath, origin).then(
    ...
    function createOrGetResourceMonitorAE(target, ae) {
      return new Promise(function (resolve, reject) {
        ...
        onem2mService
          .createResource(parentResourcePath, ae, origin)
          .then(function (monitorAERes) {
            ...
            if (aeID && aeID.trim().length > 0) {
              ...
              onem2mService
                .getResource(resourceUrl, cseOrigin)
                .then(function (monitorAERes) {
                  resolve(monitorAERes);
                })
                .catch(function (err) {
    ...
    function getBaseResources(target, ae) {
      return new Promise(function (resolve, reject) {
        ...
        for (var i = 0; i < target.aryPaths.length; i++) {
          ...
          getResourceWork.push(
            onem2mService.getResource(resourceUrl, acpOrigin)
          );
        }

        try {
          Promise.all(getResourceWork).then(
            function (resources) {
              resolve(resources);
            },
            function (reason) {
    ...
    function discoverChildResource(
      resourceUrl,
      parentType,
      origin,
      node,
      ...
      onem2mService
        .discoverResource(resourceUrl, parentType, origin, undefined, 5, offset) //  5 는 화면에 보여줄 최대 CIN의 갯수
        .then(function (res) {
    ...
    function createContainer() {
      return new Promise(function (resolve, reject) {
        ...
        //  try to create resource monitor AE resource
        onem2mService
          .createResource(parentResourcePath, ae, acpOrigin)
          .then(function (res) {
            //  success to create resource monitor AE
            //  check aeid
    ...
    function zoomed() {
      if (d3.event && d3.event.type == "zoom") return;
    ...
    function initDemoTree() {
      ...
      $scope.zoom = d3.behavior.zoom().scaleExtent([0.3, 8]).on("zoom", zoomed);

      $scope.tree = d3.layout
        .tree()
        //.nodeSize([40, 200])
        .size([height, width])
        .children(function (d) {
      ...
      filterBlur.append("feGaussianBlur").attr("stdDeviation", "1.1");
          ...
          ModalService.showModal({
            ...
            modal.close.then(function (result) {
              if (result) {
                ...
                var resourceUrl =
                  resourceMonitor.monitoringTarget.baseUrl + resourcePath;
                var acpOrigin = resourceMonitor.acpOrigin;

                onem2mService
                  .createResource(resourceUrl, newResource, acpOrigin)
                  .then(function (res) {
                    var newResourceType = _.keys(res)[0];
                    var newNode = result.parent.appendChild(
                      res[newResourceType].rn
                    );
    ...
    var showResourcePropertyPanel = function () {
      return function (node) {
        if (node && node.data) {
          ...
          node.popupDeleteAuthority =
            node.depth < resourceMonitor.targetResourceDepth ? false : true;

          var eventOffsetX = 100; // node.x;
          var eventOffsetY = 100; // node.y;
          if (d3.event instanceof TouchEvent) {
            var resourceTreeView =
              document.getElementById("resource-tree-view");
            var baseX =
              resourceTreeView.scrollLeft - resourceTreeView.offsetLeft;
    ...
    var d3TooltipBox2 = function (show, data, openCallback) {
      if (!show) {
        ...
        try {
          ...
          dtb.append("span");
          dtb.append("img").attr("src", "./images/img_tooltip.png");
        } catch (e) {
          console.log(e);
        }
        // this gets executed when a contextmenu event occurs
        return function (data) {
          try {
            ...
            if (span.text() != data.getName()) span.text(data.getName());
...
*/
        ...
        .attr("transform", function (d) {
          try {
            if (d.parent) {
              ...
              return (
                "translate(" + (source.y0 + 180) + "," + (source.x0 + 12) + ")"
              );
            }
          } catch (ex) {
      ...
      var expandMark = nodeEnter
        ...
        .style("visibility", function (d) {
          return d.hasChildren() || !d.isDiscovered() ? "visible" : "hidden";
        })
        .on("click", click);
      expandMark
        .append("circle")
      ...
      var nodeUpdate = node
        ...
        .attr("class", function (d) {
          ...
          if (d) {
            ...
            if (d.selected) className += " selected";
            if (d.updated) className += " updated";
            if (d.justupdated) className += " justupdated";
            if (d.justupdated && $scope.animation) className += " animated";
      ...
      nodeUpdate
        ...
        .style("fill", function (d) {
          return d.updated ? "#ff0000" : "#000000";
        });
      ...
      nodeUpdate
        .select("g.expand-marker")
        .style("visibility", function (d) {
          return d.isDiscovered() && !d.hasChildren() ? "hidden" : "visible";
        })
        .attr("transform", "translate(180, 12)");

      nodeUpdate.select("g.expand-marker circle").style("stroke", function (d) {
        return getResourceColor(d);
      });

      nodeUpdate
        .select("g.expand-marker line.horizontal")
        .style("stroke", function (d) {
          return getResourceColor(d);
        })
        .style("visibility", function (d) {
          return d.isDiscovered() && d.hasChildren() ? "visible" : "hidden";
        });

      nodeUpdate
        .select("g.expand-marker line.vertical")
        .style("stroke", function (d) {
          return getResourceColor(d);
        })
        .style("visibility", function (d) {
          return d.isDiscovered() && d.hasChildren() && d.isCollapsed()
            ? "visible"
            : "hidden";
        });

      nodeUpdate
        .select("g.expand-marker path.arrow")
        .style("stroke", function (d) {
          return getResourceColor(d);
        })
        .style("visibility", function (d) {
          return !d.isDiscovered() ? "visible" : "hidden";
        });
      ...
      var nodeExit = node
        ...
        .attr("transform", function (d) {
          if (d.parent)
            return (
              "translate(" + (d.parent.y + 180) + "," + (d.parent.x + 12) + ")"
            );
      ...
      nodeExit.select("circle").attr("cx", 9).attr("cy", 8).attr("r", 1e-6);

      nodeExit.select("text").style("fill-opacity", 1e-6);

      var expandMark = nodeExit
        .select("g.expand-marker")
        .attr("x", 0)
        .attr("y", 0)
  ...
  function getMenuBitVector(data) {
    ...
    } else if (!menuBitVector) {
      // if typecode is 4, 5, 23 then remove create in menu
      var contentModelLength = window.OneM2M.Resource.getContentModel(
        "m2m:" + data.getTypeName()
      ).length;
