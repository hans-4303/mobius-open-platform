...
router.get("/", function (req, res, next) {
  ...
  if (fu && fu === "1") {
    if (req.query.type) {
      ...
      var contentModel =
        onem2mClient.Model.Resource.getContentModel(parentType);
      var resourceTypes = [];
      var limType = null;
      ...
      onem2mClient.Http.DiscoverDirectChildResource(
        resourceUrl,
        origin,
        resourceTypes
      )
        .then(
          function (result) {
            if (limType) {
              ...
              return onem2mClient.Http.DiscoverChildInstance(
                resourceUrl,
                origin,
                limType,
                limit
  ...
  onem2mClient.Http.CreateResource(parentResourceUrl, resource, origin)
    ...
    .catch((err) => {
      if (err.statusCode == 409) {
        if (nodeType == "m2m:sub") {
          //중복된 대상이 SUB일 때만 삭제 및 재생성을 진행

          console.log(
            `try delete existed subscription : ${parentResourceUrl}/${rn}`
          );
          onem2mClient.Http.DeleteResource(`${parentResourceUrl}/${rn}`, origin)
            .then(() => {
              return onem2mClient.Http.CreateResource(
                parentResourceUrl,
                resource,
                origin
              );
