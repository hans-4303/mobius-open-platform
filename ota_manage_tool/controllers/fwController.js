...
exports.setBlockSize = (req, resp) => {
  ...
  if (
    isNaN(newSize) ||
    newSize < SEND_BUFFER_LOWER_BOUND ||
    newSize > SEND_BUFFER_UPPER_BOUND
  ) {
    // resp.set({'Content-Type': 'text/plain', 'Encoding': 'utf8'});
    resp
      .status(400)
      .send(
        "Fail to change buffer size: '" +
          newSize +
          "' is illegal value! (" +
...
exports.getSize = (req, resp) => {
  if (
    req.params.hasOwnProperty("aeid") &&
    req.params.hasOwnProperty("version")
  ) {
    var aename = req.params.aeid;
    var size;

    imgSrevice.getImageByAEID(aename).then((aeDoc) => {
      if (aeDoc) {
        try {
          size = aeDoc.fileSize;
            ...
            if (stat) {
              ...
              if (req.query.hasOwnProperty("seq")) {
                try {
                    ...
                    fs.open(filePath, "r", function (status, fd) {
                      ...
                      fs.read(
                        ...
                        function (err, num, buff) {
                          ...
                          logger.info(
                            `[Patch] : ${aename} send file (${seq}/${loop_time}) size : ${buffer.length} | global : ${global.send_buffer_size}`
                          );
                  ...
                  } else if (seq == loop_time + 1) {
                    // seq가 loop_time + 1이라는 것은, 마지막 파트인 것
                    // 파일의 잔여 내용의 길이를 구한다.
                    ...
                    fs.open(filePath, "r", function (status, fd) {
                      ...
                      fs.read(
                        ...
                        function (err, num, buff) {
                          ...
                          logger.info(
                            `[Last Patch] : ${aename} send file (${seq}/${loop_time}) size : ${buffer.length} | global : ${global.send_buffer_size}`
                          );
                          return;
                        }
                      );
              ...
              failResponse(
                resp,
                new Error("Could not find firmware file!"),
                "could not find file!",
                aename
              );
              return;
