...
exports.addAE = (req, res) => {
  if (
    !hasOwnProperties(req.body, "aeid", "version") ||
    !hasOwnProperties(req, "file")
  ) {
    ...
    aeMgmtService
      ...
      .catch((err) => {
        failResponseMessage(
          ...
          err,
          err.message ||
            "AE 등록에 실패했습니다. 반복시 관리자에게 문의 바랍니다."
        );
      });
  } catch (err) {
    failResponseMessage(
    ...
    aeMgmtService
      ...
      .catch((err) => {
        failResponseMessage(
          res,
          err,
          "AE 수정에 실패했습니다. 반복시 관리자에게 문의 바랍니다."
        );
      });
  } catch (err) {
    failResponseMessage(
...
exports.removeAE = (req, res) => {
  ...
  try {
    ...
    failResponseMessage(
      res,
      err,
      "AE 삭제에 실패했습니다. 반복시 관리자에게 문의 바랍니다."
    );
  }
};
...
exports.getAE = (req, res) => {
  let aeID = req.body.aeID
    ? req.body.aeID
    : failResponseMessage(res, null, "Could not get AE ID");

  try {
    aeMgmtService.getAE(aeID).then((result) => {
      if (result) {
        res.status(200).json({
    ...
    failResponseMessage(
      res,
      err,
      "AE를 불러오지 못했습니다. 반복시 관리자에게 문의 바랍니다."
    );
  }
};
    ...
    aeMgmtService
      ...
      .catch((err) => {
        failResponseMessage(
          res,
          err,
          "AE를 가져오지 못했습니다. 반복시 관리자에게 문의 바랍니다."
        );
      });
  } catch (err) {
    failResponseMessage(
...
exports.updatePatchDate = (req, res) => {
  ...
  aeMgmtService
    .updatePatchDate(email, targetAE)
    .then((result) => {
      if (result) {
        res.status(200).json({
          message: "well done",
