...
exports.createUser = (req, res) => {
  if (!hasOwnProperties(req.body, "email", "password", "name")) {
    ...
    try {
      ...
      onem2mServcie
        .createOrGetAEResource(user.email)
        .then((aeResource) => {
          if (aeResource) {
    ...
    try {
      ...
      userMgmtService
        ...
        .catch((err) => {
          failResponseMessage(
            res,
            "탈퇴 처리도중 장애가 발생했습니다. 잠시후 다시 시도해주세요"
          );
        });
    } catch (err) {
      failResponseMessage(
  ...
  userMgmtService
    ...
    .catch((err) => {
      failResponseMessage(
        res,
        err,
        "사용자를 삭제하는데 실패했습니다. 지속시 관리자에게 문의 바랍니다."
      );
    });
};
