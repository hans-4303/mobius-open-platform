...
export class AeListComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.authService.userAuthentiate().subscribe((rs) => {
      this.aeMgmtService.getAEListByUser(userEmail).subscribe(
        (res) => {
        ...
        this.aeMgmtService.deleteAE(row.aeid).subscribe(
          ...
          (err) => {
            this.notifier.notify(
              "error",
              err.error.message ||
                "AE 삭제에 실패했습니다. 지속시 관리자에게 문의 바랍니다."
            );
            console.error(err);
          }
        );
  ...
  openUploadDialog(row, i) {
    this.dialog
      ...
      .subscribe((data) => {
        ...
        this.aeMgmtService
          .uploadNewFirmware(row.aeid, data.version, data.file)
          .subscribe(
            (res: any) => {
  ...
  openDeleteManyAlert() {
    ...
    let DialogRef = this.openAlert(
      `${checkNum}개의 AE 삭제 작업을 수행하시겠습니까?`
    );
    DialogRef.afterClosed().subscribe(
      (res) => {
        if (res) {
          ...
          this.aeMgmtService.deleteAE(selectedAEs).subscribe(
            (rs) => {
              if (!rs) {
                return;
              }
    ...
    this.dialog
      ...
      .subscribe(
        (res) => {
          ...
          this.ngxService.show();
          this.aeMgmtService.postNewAE(res, res.file).subscribe(
            (res: any) => {
              let data = res.data;
              ...
              this.notifier.notify(
                "success",
                res.message || `성공적으로 ${data.aeid}를 저장했습니다.`
              );
            },
            (err) => {
              this.ngxService.hide();
