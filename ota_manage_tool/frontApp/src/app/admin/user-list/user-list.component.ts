...
export class UserListComponent implements OnInit {
    ...
    this.dialog
      ...
      .subscribe((res) => {
        if (res) {
          ...
          this.userMgmtSevice.userDeleteRequset(target).subscribe(
            (res) => {
              // 삭제 대상이 단수일 경우
  ...
  clickControlPacketLength() {
    this.userMgmtSevice.getPacketSize().subscribe(
      (res) => {
        if (res.bufferSize) {
          ...
          this.packetDialog
            ...
            .subscribe(
              (res) => {
                if (res) {
                  var packetSize = res;
                  this.userMgmtSevice.setPacketSize(packetSize).subscribe(
                    (res) => {
                      this.notifier.notify(
                        "success",
                        `buffer size  changed to ${packetSize}`
                      );
                    },
                    (err) => {
