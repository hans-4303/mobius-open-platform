...
function _byMonth() {
  ...
  var beginDate = new Date(
    parseInt(this.beginDate / MS_PER_A_MONTH) * MS_PER_A_MONTH
  );
...
function _byDate() {
  ...
  var beginDate = new Date(
    parseInt(this.beginDate / MS_PER_A_DAY) * MS_PER_A_DAY
  );
  var endDate = new Date(parseInt(Date.now() / MS_PER_A_DAY) * MS_PER_A_DAY);

  for (var from = beginDate; from <= endDate; ) {
    var timeSlotStr =
      _padding(from.getFullYear(), 4) +
      _padding(from.getMonth() + 1, 2) +
      _padding(from.getDate(), 2);
    labels.push(timeSlotStr);
