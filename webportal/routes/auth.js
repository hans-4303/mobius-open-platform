  ...
  passport.authenticate(
    ...
    function (err, user, info) {
      ...
      req.login(user, function (err) {
        ...
        Promise.all([acTokenPromise, reTokenPromise]).then((values) => {
          var acToken = values[0];
          var reToken = values[1];
          if (!acToken || !reToken) {
            return res
              .status(500)
              .send({ message: "로그인 도중 장애가 발생헀습니다." });
          } else {
  ...
  passport.authenticate(
    ...
    function (err, user, info) {
      ...
      req.login(user, function (err) {
        ...
        tokenMakeService.createAccessToken(loginUser).then((acToken) => {
          if (!acToken) {
            return res
              .status(500)
              .send({ message: "로그인 도중 장애가 발생헀습니다." });
...
router.post("/password/reset-token", function (req, res, next) {
  ...
  userManager.requestToResetPassword(requestUserEmail).then(
    function (result) {
...
router.put("/password", function (req, res, next) {
  ...
  userManager.updateUserPassword(resetToken, password, password2).then(
    function (result) {
      res.status(result.statusCode).send(result.data);
    },
    function (err) {
...
router.post("/re/check", function (req, res, next) {
  ...
  tokenMgmtService
    .findTokenExist(email, oceanReToken)
    .then(() => {
      res.status(200).json({
        result: true,
      });
