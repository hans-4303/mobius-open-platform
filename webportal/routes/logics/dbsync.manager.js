...
function __mqttMessageHandler(topic, message) {
  ...
  if (!__notificationMessageHandler(notiEventType, primitiveContent)) return;
...
function __notificationMessageHandler(notiEventType, primitiveContent) {
  try {
    ...
    if (surTokens[surTokens.length - 1] != WEBPORTA_SUB_NAME) return false;
      ...
      .then(function (deviceList) {
        deviceList.map(function (device) {
          onem2mClient.Http.SubscribeTo(
            targetCbUrl + "/" + device.resourceInfo.resourceName,
            WEBPORTA_SUB_NAME,
            device.owner.email,
            MQTT_URL
          )
...
function _synchronizeDeviceAcps(deviceResourceId) {
  ...
  return new Promise(function (resolve, reject) {
    try {
      ...
      DevicesModel.findOne({ "resourceInfo.resourceId": deviceResourceId })
        .populate("owner")
        .exec()
        ...
        .then(function (device) {
          ...
          return onem2mManager.getAeResource(
            _user,
            _device.resourceInfo.resourceName
          );
        })
          ...
          return Promise.all(
            acpiArray.map(function (acpi) {
              return new Promise(function (resolve, reject) {
                try {
                  onem2mManager
                    .getResourceByResourceId(_user, acpi)
                    .then(function (acp) {
