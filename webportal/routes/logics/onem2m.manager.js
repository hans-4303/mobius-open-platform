          ...
          .then(
            ...
            function (error) {
              ...
              if (error.statusCode === 409) {
                var aeUrl = __getResourceUrl(
                  "/" + TARGET_IOT_PLATFORM_CB_NAME + "/" + aeName
                );
...
function __createUserACP(origin, userAe) {
  ...
  return new Promise(function (resolve, reject) {
    try {
      ...
      var targetAeUrl = __getResourceUrl(
        "/" + TARGET_IOT_PLATFORM_CB_NAME + "/" + userAe["m2m:ae"].rn
      );
      ...
      onem2mClient.Http.CreateResource(targetAeUrl, acpObject, origin)
        ...
        .then(
          function (acpObj) {
            ...
            return onem2mClient.Http.UpdateResource(
              targetAeUrl,
              aeUpdate,
              origin
            );
          ...
          function (error) {
            if (error.statusCode === 409) {
              ...
              debug(
                "Failed to create User ACP(error while create ACP resource)",
                error
              );
...
function __getUserACP(origin, userAe) {
  ...
  return new Promise(function (resolve, reject) {
    try {
      var targetAcpUrl = __getResourceUrl(
        "/" +
          TARGET_IOT_PLATFORM_CB_NAME +
          "/" +
          userAe["m2m:ae"].rn +
          "/" +
          acpName
...
function __createACPResource(origin, parentAe, acpInfo) {
  return new Promise(function (resolve, reject) {
    try {
      ...
      var targetAeUrl = __getResourceUrl(
        "/" + TARGET_IOT_PLATFORM_CB_NAME + "/" + parentAe["m2m:ae"].rn
      );
...
function __insertAccessControlRule(acpObj, acRule) {
  ...
  if (resType != "m2m:acp")
    throw new Error(
      ...
      'Argument must a "ACP" resource object'
    );

  if (!acpObj[resType]["pv"]) acpObj[resType]["pv"] = { acr: [] };

  if (!acpObj[resType]["pv"]["acr"]) acpObj[resType]["pv"]["acr"] = [];
...
function __findAccessControlRule(acpObj, acrIndexOrAcorId) {
  ...
  if (!acpObj[resType]["pv"]) return null;

  if (!acpObj[resType]["pv"]["acr"]) return null;
  ...
  if (typeof acrIndexOrAcorId === "string") {
    acpObj[resType]["pv"]["acr"].map(function (acr) {
      acr.acor.map(function (acor) {
        if (acor == acrIndexOrAcorId) result = acr;
      });
    });
    ...
    if (
      acrIndexOrAcorId < 0 ||
      acrIndexOrAcorId >= acpObj[resType]["pv"]["acr"].length
    )
      throw new Error(
        "Illegal argument exception",
...
function __deleteAccessControlRule(acpObj, acrIndex) {
  ...
  if (!Array.isArray(acpObj[resType]["pv"]["acr"]))
    throw new Error(
      "Illegal argument exception",
      "acr of Target ACP is not array"
    );
  ...
  if (typeof acrIndex === "number") {
    ...
    acpObj[resType]["pv"]["acr"].map(function (acr) {
      acr.acor = acr.acor.filter(function (acor) {
        if (acor != acrIndex) return true;
        else return false;
      });
    });

    acpObj[resType]["pv"]["acr"] = acpObj[resType]["pv"]["acr"].filter(
      function (acr) {
        if (acr.acor.length > 0) return true;
        else return false;
      }
    );
  }
...
function __setAcpiToResource(
  origin,
  targetResourceIdOrPath,
  acpId,
  byTargetOrigin
) {
  var acpiList = [];
  if (Array.isArray(acpId)) acpiList = acpId;
  else acpiList = [acpId];
    ...
    try {
      ...
      onem2mClient.Http.GetResource(targetResourceUrl, origin)
        ...
        .then(
          ...
          function (error) {
            debug(
              "Failed to set acpi to target resource.(error while get target resource data)",
              error
            );
            reject(error);
          }
        ...
        .then(
          ...
          function (error) {
            debug(
              "Failed to set acpi to target resource(error while update target resource data)",
              error
            );
            reject(error);
          }
    ...
    try {
      ...
      onem2mClient.Http.GetResource(targetResourceUrl, origin)
        ...
        .then(
          ...
          function (error) {
            debug(
              "Failed to delete acpi to a resource(error while get target resource data)",
              error
            );
            reject(error);
          }
        ...
        .catch(function (error) {
          debug(
            "Failed to delete acpi to a resource(error while update target resource)",
            error
          );
          reject(error);
        });
    ...
    try {
      ...
      __getUserAeResource(user)
        .then(
          ...
          function (err) {
            debug(
              "Failed to list child resources(error while get parent resource data)",
              parentUrl,
              origin,
              err
            );
        ...
        .then(
          function (parentResource) {
            ...
            var contentModel =
              onem2mClient.Model.Resource.getContentModel(resourceType);

            contentModel = contentModel.filter(function (item) {
              if (BROWSERABLE_TYPES.indexOf(item) == -1) return false;
              else return true;
            });

            return Promise.all(
              contentModel.map(function (type) {
                return new Promise(function (resolve, reject) {
                  try {
                    var limit = undefined;
                    if (type === "4" || type === "26") {
                    ...
                    onem2mClient.Http.DiscoverResource(
                      parentUrl,
                      origin,
                      type,
                      limit
          ...
          function (err) {
            debug(
              "Failed to list child resources(error while discover child resources)",
              err
            );
        ...
        .then(
          ...
          function (err) {
            debug(
              "Failed to list child resources(while collect child resource urls)",
              err
            );
            reject(err);
          }
        ...
        .then(function (uriList) {
          ...
          return Promise.all(
            uriList.map(function (uri) {
              ...
              if (resourceUrl === parentUrl) return Promise.resolve(null);
        ...
        .then(function (resourceList) {
          ...
          resourceList.map(function (item) {
            if (item && item.success) {
              ...
              var resourceType = onem2mClient.Model.Resource.getShortName(
                item.data
              );
              ...
              resultList.push({
                ...
                content:
                  resourceType == "cin" ? item.data[qualifiedName]["con"] : "",
        ...
        .catch(function (err) {
          debug(
            "Failed to list child resources(while collect child resource data)",
            err
          );
    ...
    try {
      ...
      __getUserAeResource(user)
        .then(
          ...
          function (err) {
            debug(
              "Failed to get AE resource(error while get user AE resource data)",
              aePath,
              origin,
              err
            );
        ...
        .catch(function (err) {
          debug(
            "Failed to get AE resource(error while get AE resource data)",
            aePath,
            origin,
            err
          );
    ...
    try {
      ...
      __getUserAeResource(newUser)
        .then(
          ...
          function (err) {
            debug(
              "Fail to initialize new user(error while get user ae resource)",
              err
            );
            reject(err);
          }
        ...
        .catch(function (err) {
          debug(
            "Fail to initialize new user(error while create user ACP resource)",
            err
          );
          reject(err);
        });
    ...
    try {
      ...
      __getUserAeResource(_user)
        .then(
          ...
          function (err) {
            debug(
              "Fail to grand access rights(error while get user AE resource data)",
              err
            );
            reject(err);
          }
        ...
        .catch(function (err) {
          debug(
            "Fail to grand access rights(error while set acpi to target resource)",
            err
          );
          reject(err);
        });
  ...
  return new Promise(function (resolve, reject) {
    try {
      ...
      __getUserAeResource(_user)
        .then(
          function (aeObj) {
            return __deleteAcpiFromResource(
              user.email,
              targetResourceId,
              acpId
            );
          ...
          function (err) {
            debug(
              "Fail to revoke access rights(error while set acpi to target resource)",
              err
            );
        ...
        .catch(function (err) {
          debug(
            "Fail to revoke access rights(error while delete acpi from target resource)",
            err
          );
          reject(err);
        });
    ...
    try {
      ...
      __getUserAeResource(user)
        .then(
          ...
          function (err) {
            debug(
              "Fail to get user ACP resource(error while get user ae resource)",
              err
            );
            reject(err);
          }
        ...
        .catch(function (err) {
          debug(
            "Fail to get user ACP resource(error while get user acp resource data)",
            err
          );
          reject(err);
        });
    ...
    try {
      ...
      __getUserAeResource(user)
        .then(
          ...
          function (err) {
            debug(
              "Fail to get resource(error while get user ae resource)",
              err
            );
            reject(err);
          }
  ...
  return new Promise(function (resolve, reject) {
    try {
      ...
      __deleteResource(user.email, resourceId)
        ...
        .catch(function (err) {
          debug(
            "Fail to delete device resource(error while delete device resource)",
            err
          );
          reject(err);
        });
    ...
    try {
      ...
      __getUserAeResource(_user)
        .then(
          ...
          function (err) {
            debug(
              "Fail to add acr to ACP resource(error while get user ae resource)",
              err
            );
            reject(err);
          }
        ...
        .then(
          function (acpObj) {
            ...
            if (acr) {
              ...
              var acor = [];
              if (typeof rule.acor === "string") acor = [rule.acor];
              else if (Array.isArray(rule.acor)) acor = rule.acor;
          ...
          function (err) {
            debug(
              "Fail to add acr to ACP resource(error while get ACP resource data)",
              err
            );
            reject(err);
          }
        ...
        .catch(function (err) {
          debug(
            "Fail to add acr to ACP resource(error while update ACP resource)",
            err
          );
          reject(err);
        });
...
function _deleteAcrFromACP(user, acpName, index) {
  ...
  if (isNaN(_acrIndex)) _acrIndex = index;
    ...
    try {
      ...
      __getUserAeResource(_user)
        .then(
          ...
          function (err) {
            debug(
              "Fail to delete acr from ACP resource(error while get user AE resource)",
              err
            );
            reject(err);
          }
        ...
        .then(
          ...
          function (err) {
            debug(
              "Fail to delete acr from ACP resource(error while get ACP resource data)",
              err
            );
            reject(err);
          }
        ...
        .catch(function (err) {
          debug(
            "Fail to delete acr from ACP resource(error while update ACP resource)",
            err
          );
          reject(err);
        });
    ...
    try {
      ...
      __getUserAeResource(_user)
        .then(
          ...
          function (err) {
            debug(
              "Fail to update acr to ACP resource(error while get user ae resource)",
              err
            );
            reject(err);
          }
        ...
        .then(
          ...
          function (err) {
            debug(
              "Fail to update acr to ACP resource(error while get ACP resource data)",
              err
            );
            reject(err);
          }
        ...
        .catch(function (err) {
          debug(
            "Fail to update acr to ACP resource(error while update ACP resource)",
            err
          );
          reject(err);
        });
    ...
    try {
      ...
      __getUserAeResource(user)
        .then(
          ...
          function (error) {
            debug(
              "Failed to create new ACP resource(error while get user AE resource)",
              error
            );
            reject(error);
          }
        ...
        .catch(function (error) {
          debug(
            "Failed to create new ACP resource(error while create new ACP resource)",
            error
          );
          reject(error);
        });
      ...
      __getUserAeResource(user)
        .then(
          function (aeObj) {
            ...
            parentUrl = __getResourceUrl(
              "/" + TARGET_IOT_PLATFORM_CB_NAME + "/" + aeObj[resourceType].rn
            );
          ...
          function (err) {
            debug(
              "Failed to list user ACP resources(error while get user AE resource)",
              err.message
            );
            reject(err);
          }
        ...
        .then(
          ...
          function (err) {
            debug(
              "Failed to list user ACP resources(error while get parent resource data)",
              err
            );
            reject(err);
          }
        ...
        .then(
          ...
          function (err) {
            debug(
              "Failed to list user ACP resources(error while discover child ACP resources)",
              err
            );
            reject(err);
          }
        ...
        .then(
          ...
          function (err) {
            debug(
              "Failed to list user ACP resources(error while collect child ACP resource urls)",
              err
            );
            reject(err);
          }
        ...
        .catch(function (err) {
          debug(
            "Failed to list user ACP resources(error while collect child ACP resource data)",
            err
          );
          reject(err);
        });
