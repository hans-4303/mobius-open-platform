...
function _findUsersByEmail(emailArray) {
  return new Promise(function (resolve, reject) {
    try {
      ...
      UsersModel.find({ email: { $in: emailArray } }, { email: 1, username: 1 })
        .exec()
...
function _registerNewUser(user) {
  return new Promise(function (resolve, reject) {
    try {
      ...
      UsersModel.count({ email: user.email })
        .exec()
        ...
        .then(
          function (userCount) {
            if (userCount > 0) {
              ...
              if (!user.registerType) {
                ...
                _newUser.registerType = user.registerType
                  ? user.registerType
                  : "";
              }
          ...
          function (err) {
            debug(
              "Fail to register new user.(while check whether email is already used",
              err
            );
            reject(err);
          }
        ...
        .then(
          ...
          function (err) {
            debug(
              "Fail to register new user.(while creating user AE resource)",
              err
            );
            reject(err);
          }
        ...
        .then(
          ...
          function (err) {
            debug(
              "Fail to register new user.(while saving new user info to database)",
              err
            );
            reject(err);
          }
        ...
        .then(
          ...
          function (err) {
            debug(
              "Fail to register new user.(while creating default user ACP resource)",
              err
            );
            reject(err);
          }
        ...
        .then(
          ...
          function (err) {
            debug(
              "Fail to register new user.(while saving ACP information to database)",
              err
            );
            reject(err);
          }
...
function __generatePasswordResetEmail(user) {
  ...
  mailContents +=
    "비밀번호 재설정 링크: http://portal.iotocean.org/#!/password-change/" +
    user.resetPassordInfo.token;
...
function _updateUserPassword(token, password, password2) {
  ...
  return new Promise(function (resolve, reject) {
    try {
      ...
      UsersModel.findOne({ "resetPassordInfo.token": token })
        .exec()
