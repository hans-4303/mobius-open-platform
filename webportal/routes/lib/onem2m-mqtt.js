(function (serverAddress) {
  ...
  CSE) on this hop, omitting any leading
    ...
    function mqttMessageHandler(topic, message) {
      ...
      if (mesgObj == null) return;

      var primitiveParameters = null;
      if (!mesgObj["m2m:rqp"])
        //  message가 request primitive인 경우만 처리
        primitiveParameters = mesgObj;
      else primitiveParameters = mesgObj["m2m:rqp"];
  ...
  MqttClient.prototype.off = function (event, handler) {
    if (this.eventHandlers[event]) delete this.eventHandlers[event];
  };
  ...
  function getClient(to, from) {
    ...
    if (to.startsWith("mqtt://")) mqttServerAddress = to;
    else mqttServerAddress = "mqtt://" + to;
  ...
  function GetResource(resourceUrl, origin) {
    debug(
      "GetResource is called with url: " + resourceUrl + ", origin: " + origin
    );
  ...
  function DiscoverResource(resourceUrl, origin, type) {
    debug(
      "DiscoverResource is called with url: " +
        resourceUrl +
        ", origin: " +
        origin +
        ", type: " +
    ...
    return new Promise(function (resolved, rejected) {
      ...
      if (type) {
        ...
        if (resourceTypeCode) filterCriteria += "&ty=" + resourceTypeCode;
      }
  ...
  function CreateResource(parentResourceUrl, resource, origin) {
    debug(
      "CreateResource is called with parentResourceUrl: " +
        parentResourceUrl +
        ", resource: " +
        resource +
        ", origin: " +
    ...
    return new Promise(function (resolved, rejected) {
      ...
      var options = {
        ...
        headers: {
          Accept: "application/json",
          "Content-Type":
            "application/vnd.onem2m-res+json;ty=" + resourceTypeCode,
          "X-M2M-RI": Uuid.v4(),
          "X-M2M-Origin": origin,
        },
        body: resource,
      ...
      Http(options)
        ...
        .catch(function (error) {
          ...
          debug(JSON.stringify(error, true));
          rejected(error);
        });
    });
  }
