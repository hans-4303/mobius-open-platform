...
router.post("/:acpName/rule", function (req, res) {
  //  check exists
  onem2mManager
    .addAcrToACP(req.user, req.params.acpName, req.body)
...
router.delete("/:acpName/rule/:index", function (req, res) {
  //  check exists
  onem2mManager
    .deleteAcrFromACP(req.user, req.params.acpName, req.params.index)
...
router.put("/:acpName/rule/:index", function (req, res) {
  //  check exists
  onem2mManager
    .updateAcrToACP(req.user, req.params.acpName, req.params.index, req.body)
    .then(
      function (acpObj) {
        res.status(200).send(acpObj);
      },
...
router.post("/", function (req, res) {
  //  check exists
  onem2mManager.createNewACPResource(req.user, req.body).then(
    function (acpObj) {
      res.status(200).send(acpObj);
    },
    function (err) {
