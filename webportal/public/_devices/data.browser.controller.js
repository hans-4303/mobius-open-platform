(function () {
  ...
  function DataBrowserController(
    ...
    function _onSelectDeviceItem(resource) {
      ...
      deviceService.browse(resourceId, resourceName).then(
    ...
    function _refreshData() {
      ...
      $scope.browserData[depth].resources.map(function (item) {
        if (item.selected) resource = item;
      });
    ...
    function _onSelectBrowserItem(depth, resource) {
      ...
      if (depth >= 0) {
        $scope.browserData[depth].resources.map(function (item) {
          if (item.resourceId == resourceId) item.selected = "selected";
          else item.selected = "";
        });
