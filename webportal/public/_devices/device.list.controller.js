  ...
  function DeviceListController(
    ...
    function _itemCommandHandler(command, device) {
      switch (command) {
        ...
        case "unregister":
          var modalOptions = {
            ...
            inputs: {
              ...
              title: "리소스 등록 취소",
              messages: [
                "선택된 디바이스의 등록이 취소되며, 등록 정보가 삭제됩니다.",
                "이 작업은 복구할 수 없습니다.",
              ],
              prompt: "계속 진행 하시겠습니까?",
            },
              ...
              if (result) {
                deviceService
                  ...
                  .then(
                    function (deviceList) {
                      $scope.$apply(function () {
                        var index = $scope.deviceList.findIndex(function (
                          item
                        ) {
                          if (item.deviceId == device.deviceId) return true;
                          else return false;
                        });
        ...
        case "delete":
          ...
          .then(function (modal) {
            ...
            modal.close.then(function (result) {
              if (result) {
                deviceService
                  .deleteDeviceResource(device.deviceId)
        ...
        case "ota":
          $state.go("main.ota-manager", {
            aeResourceName: device.resourceInfo.resourceName,
          });
          break;
      }
    }
    ...
    function _deviceListFilter(device) {
      if ($scope.deviceFilter.trim() == "") return true;
      ...
      if (device.owner) {
        if (device.owner.email) comparableString.push(device.owner.email);
        if (device.owner.userid) comparableString.push(device.owner.userid);
        if (device.owner.name) comparableString.push(device.owner.name);
      }
      ...
      comparableString.map(function (str) {
        ...
        if (strLow.indexOf(keyword) != -1) found = true;
      });
