  ...
  function DeviceEditController(
    ...
    function _headerColorClass() {
      ...
      if (
        ...
        $scope.device.deviceInfo.nickname
      )
    ...
    function _deleteAcpiFromDevice(acp) {
      deviceService.deleteDeviceAcpi($scope.device.deviceId, acp.ri).then(
        function (device) {
    ...
    function _saveDeviceInfo() {
      deviceService
        .saveDeviceInfo($scope.device.deviceId, $scope.device.deviceInfo)
        .then(
          function (device) {
            $scope.$apply(function () {
              $scope.device = device;
            ...
            .then(function (modal) {
              ...
              modal.close.then(function (result) {
                if (result && result.length > 0) {
                  deviceService
                    .updateDeviceAcpi($scope.device.deviceId, result)
                    .then(
                      function (device) {
                        $scope.$apply(function () {
                          $scope.device = device;
    ...
    function _isSaveButtonDisabled() {
      ...
      if ($scope.formDeviceNickname.deviceNickname.$dirty) formIsDirty = true;
      if ($scope.formDeviceNickname.deviceDescription.$dirty)
        formIsDirty = true;
      if ($scope.iconChanged) formIsDirty = true;

      if ($scope.formDeviceNickname.deviceNickname.$valid) formIsValid = true;
    ...
    function _init() {
      ...
      deviceService.getDeviceInfo($scope.deviceId).then(
        function (device) {
          $scope.$apply(function () {
            $scope.device = device;
            $scope.iconChanged = false;
    ...
    function _selectDeviceIcon(icon) {
      if ($scope.device.deviceInfo.icon == icon) return;
    ...
    function _acpListFilter(item) {
      if ($scope.acpFilter.trim() == "") return true;

      var keyword = $scope.acpFilter.toLowerCase();
      var rn = item.rn.toLowerCase();

      if (rn.indexOf(keyword) != -1) return true;
      ...
      if (item.lbl) {
        item.lbl.map(function (lbl) {
          ...
          if (lowLbl.indexOf(keyword) != -1) found = true;
        });
        ...
        item.pv.acr.map(function (acr) {
          if (acr.acor) {
            acr.acor.map(function (acor) {
              ...
              if (lowAcor.indexOf(keyword) != -1) found = true;
            });
          }
        });
