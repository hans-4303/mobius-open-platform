...
router.post("/:widgetId/trigger", (req, res, next) => {
  ...
  widgetManager
    .triggerResource(auth.parsed, triggerData)
    .then((widgetList) => {
...
router.delete("/:widgetId", (req, res, next) => {
  ...
  widgetManager
    .deleteWidget(auth.parsed, widgetId)
    .then((widget) => {
...
router.put("/widget-order", (req, res, next) => {
  ...
  widgetManager
    .updateWidgetOrder(auth.parsed, widgetIds)
    .then((widgetIds) => {
...
router.put("/:widgetId", (req, res, next) => {
  ...
  widgetManager
    .updateWidget(auth.parsed.userId, widgetId, updateData)
    .then((widget) => {
      res.status(200).json(widget);
    })
    .catch((ex) => {
...
router.post("/", (req, res, next) => {
  ...
  widgetManager
    .createWidget(auth.parsed, widgetInfo)
    .then((widget) => {
      res.status(200).json(widget);
    })
    .catch((ex) => {
...
router.get("/:widgetId", (req, res, next) => {
  ...
  widgetManager
    .getWidget(auth.parsed, widgetId)
    .then((widgetList) => {
      res.status(200).json(widgetList);
    })
    .catch((ex) => {
