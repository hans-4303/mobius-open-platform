  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      UserModel.findOneByUserId(userId)
        .then((userDoc) => {
          if (userDoc) {
            return DatasourcesModel.upsertDatasource(
              userDoc,
              datasourceId,
              updateData
            );
            ...
            throw new PlatformError(
              PlatformError.SOURCE.DATASOURCE.LIST,
              "unknown",
              "Invalid user info"
            );
          }
...
function installMqttEventTrigger(userId, datasourceId, triggerInfo) {
  return new Promise((resolve, reject) => {
    try {
      onem2mManager
        .subscribeToTriggerNode(userId, datasourceId, triggerInfo.resourcePath)
