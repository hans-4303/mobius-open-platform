...
function _mqttNotificationHandler(
  mqttAddress,
  from,
  topicParams,
  notiEventType,
  ...
  try {
    ...
    if (!userId.startsWith(DASHBOARD_USER_AE_PREFIX)) return false;
    ...
    var resourceTypeCode =
      onem2mClient.Model.Resource.getTypeCode(resourceType);
...
function _triggerWidget(userId, widgetId, data) {
  ...
  var codeFullPath = path.join(
    process.cwd(),
    SYSTEM_DIRECTORIES.code,
    subdir,
    widgetId + ".js"
  );
  ...
  widgetManager
    .updateWidgetData(userId, widgetId, dataset)
    .then((widgetData) => {
      var pushData = {
        widgetId: widgetId,
        widgetData: dataset,
      ...
      pushMessageManager.sendPushMessage(
        userId,
        "widgetdata.updated",
        pushData
      );
