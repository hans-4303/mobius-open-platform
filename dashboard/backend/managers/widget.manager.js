...
exports.triggerResource = (auth, triggerInfo) => {
  return new Promise((resolve, reject) => {
    onem2mManager
      .createContentInstance(auth.userId, triggerInfo.path, triggerInfo.data)
      .then(() => {
        resolve(true);
      })
    ...
    try {
      ...
      UserModel.findOneByUserId(auth.userId)
        .then((userDoc) => {
          if (userDoc) {
            ...
            throw new PlatformError(
              PlatformError.SOURCE.WIDGET.LIST,
              "unknown",
              "Invalid user info"
            );
          }
        ...
        .catch((err) => {
          ...
          if (err instanceof PlatformError) return reject(err);
          else
            return reject(
              new PlatformError(
                PlatformError.SOURCE.WIDGET.LIST,
                "unknown",
                "Fail to create widget"
              )
            );
        });
    } catch (ex) {
...
function compareTriggerInfo(a, b) {
  ...
  switch (a.type) {
    case "onem2m_trigger":
      if (a.resourcePath == b.resourcePath) return true;
      else return false;
  }
...
function installMqttEventTrigger(userId, widgetId, triggerInfo) {
  return new Promise((resolve, reject) => {
    try {
      ...
      onem2mManager
        .subscribeToTriggerNode(userId, widgetId, triggerInfo.resourcePath)
        ...
        .then(() => {
          return onem2mManager.retrieveLatestContentInstance(
            userId,
            triggerInfo.resourcePath
          );
        })
        ...
        .then((latestCin) => {
          if (
            latestCin &&
            latestCin["m2m:cin"] &&
            latestCin["m2m:cin"]["con"]
          ) {
            return mobiusDatasyncManager.triggerWidget(
              userId,
              widgetId,
              latestCin["m2m:cin"]["con"]
            );
...
function uninstallMqttEventTrigger(userId, widgetId, triggerInfo) {
  return new Promise((resolve, reject) => {
    try {
      ...
      onem2mManager
        .unsubscribeToTriggerNode(userId, widgetId, triggerInfo.resourcePath)
