    ...
    try {
      ...
      onem2mClient.Http.CreateResource(targetCbUrl, aeObject, origin)
          ...
          function (error) {
            ...
            if (error.statusCode === 409) {
              ...
              throw new PlatformError(
                PlatformError.SOURCE.M2M.USERAE,
                "id-exists",
        ...
        .catch(function (err) {
          ...
          if (err instanceof PlatformError) return resolve(err);
          else
            return reject(
              new PlatformError(
                PlatformError.SOURCE.M2M.USERAEN,
                "unknown",
                "Fail to check ID"
              )
            );
...
function _subscribeToTriggerNode(userId, widgetId, resourcePath) {
  return new Promise(function (resolve, reject) {
    try {
      ...
      onem2mClient.Http.CreateResource(targetUrl, resource, origin).then(
        function (res) {
          resolve(res);
        },
        function (err) {
...
function _retrieveLatestContentInstance(userId, resourcePath) {
  return new Promise(function (resolve, reject) {
    try {
      ...
      onem2mClient.Http.GetResource(targetUrl, origin).then(
        function (res) {
          resolve(res);
        },
        function (err) {
...
function _unsubscribeToTriggerNode(userId, widgetId, resourcePath) {
  return new Promise(function (resolve, reject) {
    try {
      ...
      onem2mClient.Http.DeleteResource(targetUrl, origin).then(
        function (res) {
          resolve(res);
        },
        function (err) {
