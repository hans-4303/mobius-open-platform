    ...
    try {
      ...
      jwt.verify(token, secret, (err, authToken) => {
        if (err) {
          ...
          req.auth = auth;
          User.findOne({ email: authToken.u_e })
            .exec()
            .then((user) => {
              if (!user) {
                return new User({
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      User.findOneByUserId(userId)
        .then((user) => {
          ...
          if (!user) {
            throw new PlatformError(
              PlatformError.SOURCE.AUTH.LOGIN,
              "no-user-id",
        ...
        .catch((err) => {
          debug.log("ERROR:", PlatformError.SOURCE.AUTH.LOGIN, err);
          if (err instanceof PlatformError) reject(err);
          else
            reject(
              new PlatformError(
                PlatformError.SOURCE.AUTH.LOGIN,
...
exports.authCheck = (req, res, next) => {
  if (!req.auth) {
    ...
    var message = "Unknown";
    if (req.auth && req.auth.error) message = req.auth.error.toString();
    ...
    http({
      ...
      .catch((err) => {
        sendError(
          res,
          err,
          "로그아웃 도중 문제가 발생했습니다. 지속시 관리자에게 문의 바랍니다."
        );
      });
  } catch (err) {
    sendError(
