  ...
  switch (widgetName) {
    ...
    case "warnings":
      ...
      for (var i = 0; i < 100; i++) {
        result.push({
          x: i,
          y: Math.abs(Math.cos(i / 10) * 0.25 * i + 0.9 - 0.4 * i),
        });
      }
      return result;
      break;
  ...
  .then((userDoc) => {
    ...
    return Promise.all(
      datasourceList.map((datasource) => {
        return DatasourceModel.createDatasource(
          _user,
          datasource.datasourceType,
          datasource.description,
          datasource.status,
          datasource.widgetData,
          datasource.triggerInfo
        );
      })
    );
  })
  ...
  .then((datasourceDocList) => {
    ...
    return Promise.all(
      widgetList.map((widget) => {
        return WidgetsModel.createWidget(
          _user,
          widget.widgetType,
          widget.title,
